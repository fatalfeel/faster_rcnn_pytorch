...
class VOC2007(DatasetBase):
    class Annotation(object):
        class Object(object):
            ...
            def __repr__(self) -> str:
                return 'Object[name={:s}, difficult={!s}, bbox={!s}]'.format(self.name, self.difficult, self.bbox)
    ...
    CATEGORY_TO_LABEL_DICT = {  'background': 0,
    ...
    def __init__(self, path_to_data_dir: str, mode: DatasetBase.Mode, image_min_side: float, image_max_side: float):
        ...
        for image_id in self._image_ids:
            ...
            '''self._image_id_to_annotation_dict[image_id] = \
            VOC2007.Annotation( filename=root.find('filename').text,
                                objects=[VOC2007.Annotation.Object( name        = next(tag_object.iterfind('name')).text,
                                                                    ...
                                                                    bbox        = BBox(left=float(next(tag_object.iterfind('bndbox/xmin')).text) - 1,
            ...
            xmlobjects  = [VOC2007.Annotation.Object(name       = next(tag_object.iterfind('name')).text,
                                                     difficult  = next(tag_object.iterfind('difficult')).text == '1',
                                                     bbox       = BBox( left    = float(next(tag_object.iterfind('bndbox/xmin')).text) - 1,
                                                                        top     = float(next(tag_object.iterfind('bndbox/ymin')).text) - 1,
                                                                        right   = float(next(tag_object.iterfind('bndbox/xmax')).text) - 1,
            ...
            self._image_id_to_annotation_dict[image_id] = VOC2007.Annotation(jpgname, xmlobjects)
